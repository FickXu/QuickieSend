...
Page({
    ...
    request('shop/activityspupagelist', params).then(res => {
      if (res.data.data && res.data.data.length > 0) {
        ...
        data.forEach(item => {
          ...
          item.spuAbstractTags = item.spuAbstract && item.spuAbstract.split(',')
        })
  ...
  shopingCart() {
    ...
    let shopcarList = wx.getStorageSync("shopcarList") || []
    // 检查购物车是否有同一单品
    let skuIndex = shopcarList.findIndex(item => this.data.shopcarDetail.skuId == item.skuId)
    if (skuIndex > -1) {
      // 存在同一单品时合并到购物车
      ...
      let params = {
        ...
        CURRENT_QUANTITY: this.data.isLimitedBuying=='true'?1:this.data.tempDetail.CURRENT_QUANTITY,
        // 实际售价为realPrice,展示价为showPrice。在商品详情中realPrice和showPrice相等时，只显示一个价格
        showPrice: this.data.isLimitedBuying==='true'?this.data.tempDetail.realPrice:this.data.shopcarDetail.showPrice,
        // 实际售价为realPrice,展示价为showPrice。在活动商品中actPrice才是实际售价
        realPrice: this.data.isLimitedBuying==='true'?this.data.tempDetail.actPrice:this.data.shopcarDetail.realPrice,
        skuId: this.data.isLimitedBuying==='true'?this.data.tempDetail.skuId:this.data.shopcarDetail.skuId,
        skuKey: this.data.isLimitedBuying==='true'?this.data.tempDetail.skuKey:this.data.shopcarDetail.skuKey,
